...
public class NormalizeImageMinMax<T extends RealType<T>> extends MultiThreadedAlgorithm {
	...
	public boolean process() {
		final T minVal = image.firstElement().createVariable();
		minVal.setReal( image.firstElement().getMinValue() );

		final T maxVal = image.firstElement().createVariable();
		maxVal.setReal( image.firstElement().getMaxValue() );

		final ComputeMinMax< T > minMax = new ComputeMinMax< T >( image, minVal, maxVal );
