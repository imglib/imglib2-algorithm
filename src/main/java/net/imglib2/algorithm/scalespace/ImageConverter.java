...
public class ImageConverter< S extends Type<S>, T extends Type<T> > implements Algorithm, MultiThreaded, Benchmark
{
	...
	protected static <T extends Type<T>> Img<T> createImageFromFactory( final ImgFactory<T> factory, final long[] size, final T type )
	{
	...
	private boolean compareStorageContainerDimensions(final Img<S> image1, final Img<T> image2) {
		boolean ok = true;
